...
-- SOURCE: SRC-001 (Azure SQL ERP)
ALTER DATABASE ERP_DB
    SET CHANGE_TRACKING = ON
    (CHANGE_RETENTION = 2 DAYS, AUTO_CLEANUP = ON);

-- Enable per INCREMENTAL table — write every table, no abbreviations
ALTER TABLE ERP.OE_ORDER_HEADERS_ALL
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE ERP.OE_ORDER_LINES_ALL
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE ERP.ADDRESSES
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE ERP.CITY_TIER_MASTER
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE ERP.MTL_SYSTEM_ITEMS_B
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE ERP.CATEGORIES
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);
...
-- SOURCE: SRC-002 (Azure SQL CRM)
ALTER DATABASE CRM_DB
    SET CHANGE_TRACKING = ON
    (CHANGE_RETENTION = 2 DAYS, AUTO_CLEANUP = ON);
...
ALTER TABLE CRM.CustomerRegistrationSource
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE CRM.INCIDENTS
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE CRM.INTERACTIONS
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);

ALTER TABLE CRM.SURVEYS
    ENABLE CHANGE_TRACKING WITH (TRACK_COLUMNS_UPDATED = ON);
