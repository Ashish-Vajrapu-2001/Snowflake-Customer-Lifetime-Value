# CLV_Analytics — Bronze Layer Deployment Guide

## Prerequisites
- Snowflake account: [SNOWFLAKE_ACCOUNT_ID]
...
### Step 1 — Snowflake Infrastructure
  snowsql -a [SNOWFLAKE_ACCOUNT_ID] -f sql/01_setup_infrastructure.sql
  Verify:
...
### Step 2 — Enable Change Tracking on Azure SQL Sources
  ...
  sqlcmd -S [HOST_ERP] -d ERP -i sql/02_enable_change_tracking.sql
  sqlcmd -S [HOST_CRM] -d CRM -i sql/02_enable_change_tracking.sql
  Verify:
    SELECT name FROM sys.change_tracking_tables
  Note: Skip for TELEPORT sources (Marketing) — Change Tracking not required.
...
### Step 4 — Monitor Initial Sync
  Watch Fivetran dashboard → Connectors → CLV_Analytics_ERP → Sync status
...
### Step 6 — Apply Governance
  ...
  snowsql -a [SNOWFLAKE_ACCOUNT_ID] -f sql/03_governance.sql
  Verify:
